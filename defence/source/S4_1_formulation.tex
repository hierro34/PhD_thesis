...
\end{itemize}}
\only<6->{
We want to integrate in time with:
\begin{itemize}
\item<7-> High-order schemes
\item<8-> Velocity-pressure segregation
\end{itemize}}
\end{frame}%----------------------------------------------------------------------------------------
\begin{frame}
...
&-DM^{-1}G\P=DM^{-1}(K+C(\U)\U- \F) + \dot{\mathbf{G}},&\mbox{in $t^{n+1}$,}
\end{align*}
with $P^*$ extrapolation from previous time steps\\
...
\item Fractional step techniques + (e.g.) BDF3 for momentum equation: [Castillo \& Codina '15]
...
\end{overprint}
\vspace{0.3cm}
\only<3>{\emph{High-order} time integration:
\begin{itemize}
...
%&\dot{\mathbf{G}} + DM^{-1}(K+C(\U))\U+DM^{-1}G\P=DM^{-1}\F.
\end{align*}
\end{overprint}
\end{frame}
%----------------------------------------------------------------------------------------
