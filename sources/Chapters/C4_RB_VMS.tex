...
%\begin{align}
%\label{eq:NS_strong_mome}
%\partial_t\u-\nu\Delta\u+\u\cdot\nabla\u+\nabla p&=\f&\mbox{in $\Omega\times(0,T)$,}\\
%\label{eq:NS_strong_inc}
%\nabla\cdot\u&=0&\mbox{in $\Omega\times(0,T)$,}
%\end{align}
%with $\f$ the force vector and $\nu$ the kinematic viscosity. Hereinafter bold characters denote vectors and tensors.
%
%Equations (\ref{eq:NS_strong_mome})-(\ref{eq:NS_strong_inc}) have to be supplied with appropriate boundary and initial conditions. The boundary $\Gamma$ is divided into the Dirichlet ($\Gamma_D$) and the Neumann ($\Gamma_N$) parts such that $\Gamma_D\cup\Gamma_N=\Gamma$ and $\Gamma_D\cap\Gamma_N=\emptyset$. Then, the boundary and initial conditions can be written as
%\begin{align}
...
%
%In order to derive the weak form of the problem (\ref{eq:NS_strong_mome})-(\ref{eq:NS_strong_Ini}) we define some notation that will be used hereafter. We denote by $L^p(\Omega)$, $1\leq p<\infty$, the spaces of functions such that their $p$-th power is absolutely integrable in $\Omega$. For the case in which $p=2$, we have a Hilbert space with scalar product 
%\begin{equation}
%\label{eq:scalar_product}
%(u,v)_\Omega\equiv(u,v):=\int_\Omega u(\x) \, v(\x)d\Omega
%\end{equation}
%and induced norm $\|u\|_{L^2(\Omega)}\equiv\|u\|=(u,u)^{1/2}$. Abusing of the notation, the same symbol as in (\ref{eq:scalar_product}) will be used for the integral of the product of two functions, even if these are not in $L^2(\Omega)$, and both for scalar and vector fields. The space of functions whose distributional derivatives up to order $m$ are in $L^2(\Omega)$ are denoted by $H^m(\Omega)$. We will focus on the case of $m=1$, which is also a Hilbert space. $H_0^1(\Omega)$ is the set of functions in $H^1(\Omega)$ that have zero trace on $\Gamma$. Furthermore, we denote by $H^{-1}(\Omega)$ the topological dual of $H_0^1(\Omega)$ and by $\left\langle\cdot,\cdot\right\rangle$ the duality pairing between $H^{-1}(\Omega)$ and $H_0^1(\Omega)$. Given a Banach space $X$,  $L^p(0,T;X)$ is the space of time dependent functions such that their $X$-norm is in $L^p(0,T)$.
%
%The weak form of the incompressible Navier-Stokes problem (\ref{eq:NS_strong_mome})-(\ref{eq:NS_strong_Ini}) consists, e.g., in finding $[\u,p]\in {L}^2(0,T;\mathcal{V}_0)\times {\cal D}'(0,T;\mathcal{Q}_0)$ (distributions in time with values in $\mathcal{Q}_0$) such that
%\begin{equation}
%\label{eq:NS_weak}
%(\partial_t\u,\v) + B(\u;[\u,p],[\v,q]) = \left<\f,\v\right> 
%\quad\quad\forall\v\in\mathcal{V}_0,\quad\forall q\in\mathcal{Q}_0,
%\end{equation}
%satisfying the initial condition (\ref{eq:NS_strong_Ini}) in a weak sense. Here $\mathcal{V}_0:={H}_0^1(\Omega)^d$, $\mathcal{Q}_0:=L^2(\Omega)/\mathbb{R}$ and the form $B({\a};[\u,p],(\v,q))$ is defined as 
%\begin{equation}
%\label{eq:bilinear}
%B(\a;[\u,p],[\v,q]):=\nu(\nabla\u,\nabla\v)+b(\a,\u,\v)-(p,\nabla\cdot\v)+(q,\nabla\cdot\u)
%\end{equation}
...
%\end{align}
%This equivalence is lost at the discrete level. The skew-symmetric form (type 2) (\ref{eq:b_skew2}) is very common when numerical analysis are presented \cite{Badia2013Convergence,Burman2009,guermond_faedogalerkin_2007} but the skew-symmetric form (type 2) (\ref{eq:b_skew1}) has important advantages when the first argument is a discontinuous function, as will be shown below.
%
%\subsection{VMS framework}
%\label{subsec:VMS_framework}
%
...
%
%The Galerkin FE approximation of (\ref{eq:NS_weak}) consists in finding $[\u_h,p_h]\in {L}^2(0,T;\mathcal{V}_{0,h})\times {\cal D}'(0,T;\mathcal{Q}_{0,h})$ such that
%\begin{equation}
%\label{eq:NS_galerkin}
...
%\end{equation}
%It is well known that (\ref{eq:NS_galerkin}) has numerical instabilities for high mesh Reynolds number problems, i.e., when the nonlinear convective term dominates the viscous term. Another drawback of that formulation is the discrete \textit{inf-sup} condition that must be satisfied by the pair $\mathcal{V}_{0,h} \times\mathcal{Q}_{0,h}$ in order to have a well-posed problem with bounded pressure. These difficulties are overcome by using the VMS approach that follows.
%
%Let us consider a two-scale decomposition of spaces $\mathcal{V}_0$ and $\mathcal{Q}_0$ such that $\mathcal{V}_0=\mathcal{V}_{0,h}\oplus\widetilde{\mathcal{V}}_0$ and $\mathcal{Q}=\mathcal{Q}_{0,h}\oplus\widetilde{\mathcal{Q}}_0$, where $\widetilde{\mathcal{V}}_0$ and $\widetilde{\mathcal{Q}}_0$ are infinite-dimensional spaces that complete the FE spaces in $\mathcal{V}_0$ and $\mathcal{Q}_0$, respectively. Hereinafter the subscript $(\cdot)_h$ will denote the FE component and the tilde $\widetilde{(\cdot)}$ the subgrid component. Applying the two-scale decomposition to (\ref{eq:NS_weak}) we obtain a discrete problem
%\begin{equation}
...
%\end{align}
%In (\ref{eq:velo_sgs})-(\ref{eq:press_sgs}) $\mathcal{P}$ denotes the projection onto the space of subscales, which is discussed below. In turn, the vector $\R$ is the residual of the Navier-Stokes equations (\ref{eq:NS_strong_mome})-(\ref{eq:NS_strong_inc}), defined as $\R=[\R_u,R_p]^T$, with
%\begin{align}
%\label{eq:Ru}
...
%\end{align}
%where $h$ is the mesh size and $c_1$ and $c_2$ are algorithmic constants. Let us comment on expression~(\ref{eq:tau_m}):
%\begin{itemize}
%\item The influence of the constants $c_1$ and $c_2$ is discussed in Section~\ref{sec:effect_const}. A theoretical way to determine them would be to impose that the numerical dissipation they introduce be equal to the molecular dissipation in turbulent regimes, as explained in~\cite{guasch-codina-13}. 
%\item The definition of $\tau_m$ in~(\ref{eq:tau_m}) is not standard, in the sense that the one used often depends on the time step size of the time discretization, $\delta t$. Instead of~(\ref{eq:tau_m}), $\tau_m^{-1}=\frac{1}{\delta t} + \frac{c_1\nu}{h^2}+\frac{c_2|\a|}{h}$ is more often considered (see, e.g., \cite{Hsu2010,gamnitzer_time-dependent_2010}). We refer to 
%Section~\ref{sec:small_time_step} for a more detailed discussion about this topic. Likewise, other expressions with the same asymptotic behavior in terms of $h$, $\nu$ and $|\a|$ can also be employed.
%\item Expression~(\ref{eq:tau_m}) corresponds to linear isotropic elements. If elements of order $p$ are used ($p$ is not the pressure, here), $c_1$ must be replaced by $c_1 p^4$ and $c_2$ by $c_2 p$. For anisotropic elements, the definition of $h$ within each element is not obvious. A possibility is explained in \cite{Principe2010}.
%\end{itemize}
%
...
%
%\subsubsection{The dynamics of the subscales}
...
%
%Equation (\ref{eq:velo_sgs}) can be analytically integrated to give
%\begin{equation}
%\label{eq:exact_sgs}
...
%
%\subsubsection{(Non)linear scale splitting}
%
...
%\begin{remark}
%\label{rem:skewsym}
%When we use the nonlinear definition for the advection velocity, $\a=\u_h+\tilde{\u}$, the skew-symmetric term \textit{type 2} (\ref{eq:b_skew2}) in the FE equation (\ref{eq:NS_discrete}) reads: 
%\begin{equation}
%\label{eq:b_skew2_aux1}
%b(\a,\u_h,\v_h)=((\u_h+\tilde{\u})\cdot\nabla\u_h,\v_h)+\frac{1}{2}(\u_h\cdot\v_h,\nabla\cdot\u_h)+\frac{1}{2}(\u_h\cdot\v_h,\nabla\cdot\tilde{\u}).
%\end{equation}
%The last term is not well-defined, since it includes derivatives of the discontinuous subscale $\tilde{\u}$. One possibility is to neglect it (as previously done with other similar terms when arriving to (\ref{eq:NS_discrete})), which implies
%\begin{equation}
%b(\a,\u_h,\u_h)=-\frac{1}{2}(|\u_h|^2,\nabla\cdot\tilde{\u}),
%\end{equation}
%the same result obtained when the non conservative form is used.
%By contrast, the skew-symmetric term \textit{type 1} in the FE equation (\ref{eq:NS_discrete}) reads
%\begin{equation}
...
%%\end{equation}
%In Subsection (\ref{subsec:result_DHIT}) we will see the influence of the two forms of the convective term on the results.
%It is worth noting that the same approximations have been introduced in all cases to implement $b(\a,\tilde{\u},\u_h)$, but these approximations are taken into account in the (usual) energy estimates of Section \ref{sec:energy}.
...
%\end{remark}
%
%\subsubsection{The space for the subscales}
%\label{sec:oss}
%The selection of the space for the approximation of the subscales determines the projection $\mathcal{P}$ appearing in the right-hand side of (\ref{eq:velo_sgs}) and (\ref{eq:press_sgs}). The first option, already considered in \cite{Hughes2000,hughes_large_2001,bazilevs_variational_2007} and named Algebraic Subgrid Scale (ASGS) in \cite{codina_stabilization_2000} is to take the subscales in the space of the residuals, that is,
...
%\end{align}
%These definitions guarantee the orthogonality between the FE and subscale spaces in the case of static subscales, that is, neglecting temporal derivatives in (\ref{eq:velo_sgs}). It then follows that the term containing the temporal derivative of the subscale in the FE equation (\ref{eq:NS_discrete}) also vanishes.
%
%However, if the dynamic version of the method is used, the weight of the projection (\ref{eq:Pi_tau_m}) must be conveniently modified to ensure the mentioned orthogonality. As it can be seen in (\ref{eq:exact_sgs}), the definition of the weight depends on the time integration strategy, as explicitly stated in Section \ref{sec:discrete}.
%
%% {\color{blue}However, if the dynamic version of the method is used (see below), the fine scale equation (\ref{eq:velo_sgs}) should be modified in order to ensure the mentioned orthogonality. As the projection $\Pi_m$ is time-dependent since $\tau_m$ depends on time, the velocity subscale time derivative can not belong to the orthogonal subspace $\mathcal{V}_h^\bot$. Then, we must project the time derivative if we want to recover the orthogonality achieved in the static definition.
%% \begin{equation}
%% \label{eq:velo_sgs_projected}
%% \mathcal{P}\left(\partial_t\tilde{\u}\right)+\tau_m^{-1}\tilde{\u}=\mathcal{P}(\R_u).
%% \end{equation}
%% Further details will be given in section \ref{sec:energy}, where the energy conservation of the method is discussed.}
%
%% {\color{red}OJO!!! si no estoy equivocado, este argumento garantiza que $(\mathcal{P}(\mathcal{L})+\mathcal{L}^*,\tilde{\u})=0$ a nivel semidiscreto, pero se carga la condicion $(\partial_t\tilde{\u},\u_h)=0$, no??}
%
%%this definition should be changed modifying the weight in a way that depends on the time integration scheme. For example if equation (\ref{eq:velo_sgs}) is discretized as $\tilde{\u}^{n+1}=\tau_m \mathcal{P}(\R_u) + \frac{1}{\delta t}\tilde{\u}^n$ the projection \ref{eq:Pi_tau_m} must be weighted using $\left(\tau_m^{-1}+{\delta t}^{-1}\right)^{-1}$ instead of  $\tau_m$. It is important to remark that using the weighted projections the residual of the monentum equation does depend on $\partial_t\u_h$. {\color{blue} Oriol, como esta en el codigo? Conservamos la derivada temporal al calcular las proyecciones?}
%
...
%
%Taking $\v_h=\u_h$ and $q_h=p_h$ in (\ref{eq:NS_discrete}) we have the energy balance on the FE component
%\begin{align}
%\label{eq:FE_balance2}
...
%\end{align}
%In equation (\ref{eq:FE_balance2}) we group the terms as
%% \begin{center}
%% \begin{tabular}{rll}
...
%\end{tabular}
%\end{center}
%Multiplying (\ref{eq:velo_sgs}) by $\tilde{\u}$ and (\ref{eq:press_sgs}) by $\tilde{p}$, integrating over the domain and decomposing the residual of the momentum equation as $\R_u=\f-\partial_t\u_h-\mathcal{L}_{\a}(\u_h,p_h)$, % we have from (\ref{eq:sgs_balance1}) 
%we obtain the global energy balance on the fine scale
%\begin{align}
%\label{eq:sgs_balance2}
...
%\end{align}
%% In equation (\ref{eq:sgs_balance2}) we group the terms as
%% \begin{center}
%% \begin{tabular}{rll}
...
%% \end{tabular}
%% \end{center}
%We group the terms in (\ref{eq:sgs_balance2}) as
%\begin{center}
%\begin{tabular}{rll}
...
%\end{tabular}
%\end{center}
%Finally, adding up equations (\ref{eq:FE_balance2}) and (\ref{eq:sgs_balance2}) 
...
%
%In this case the energy balance for the subscale is meaningless because there are explicit expressions for the subscales (\ref{eq:static_sgs}) and (\ref{eq:press_sgs}). When (\ref{eq:static_sgs}) and (\ref{eq:press_sgs}) are used in (\ref{eq:FE_balance2}), we obtain
%\begin{align}
...
%
%For the OSS method, it is proved in \cite{guasch-codina-13} that the dissipative structure of the discrete problem has the same statistical behavior in fully developed turbulence than the continuous problem, in the sense that this dissipation has the same estimates as the molecular one. Both dissipations could be made equal by a proper choice of the stabilization parameters in (\ref{eq:tau_m}). This, however, requires a small change in the advection velocity of this expression, which depends on an integral length of the problem. See \cite{guasch-codina-13} for details.
%
...
%Using (\ref{eq:velo_sgs})-(\ref{eq:press_sgs}), we get:
%\begin{align}
%\label{eq:transfer}
%\varepsilon_{h} & = 
...
%
%Another important point of (\ref{eq:total_balance_ASGS2}) is that it immediately shows that when the mesh is fine enough, i.e., 
%\begin{equation*}
...
%\end{align}
%which implies that the last term on the left hand side of (\ref{eq:total_balance}) vanishes and that the previous one can be written as
%\begin{align}
%\left(\mathcal{P}(\mathcal{L}_{\a}(\u_h,p_h))+\mathcal{L}^*_{\a}(\u_h,p_h),\tilde{\u}\right)_h
%= \left( (\mathcal{L}_{\a}(\u_h,p_h)+\mathcal{L}^*_{\a}(\u_h,p_h),\tilde{\u}\right)_h
%=-2 \left(\nu\Delta\u_h,\tilde{\u}\right)_h
...
%% {\color{blue}Note that if the dynamic definition of the subscales is considered, the condition (\ref{eq:Pi_tau_sgs}) is achieved only if we use the modified subscale equation (\ref{eq:velo_sgs_projected}) instead of (\ref{eq:velo_sgs}). Using the definition (\ref{eq:Pi_tau_m}) we have that
%% \begin{align}
%% \label{eq:Pi_tau_sgs_demo}
...
%% & = 0.
%% \end{align}}
%Then, the global energy balance equation (\ref{eq:total_balance}) reads
%\begin{align}
%\label{eq:total_balance_OSS2}
...
%\end{align}
%which is exactly (\ref{eq:total_balance_ASGS2}) except for the projection of the force in the last term. Stability and convergence of this formulation have been proved in \cite{Badia2010,Badia2013Convergence}.
%
%%\end{itemize}
%
%
%%% \subsection{Sensitivity with respect to the stabilization parameters}
%%% \label{subsec:sensivity_tau}
%
%%% All the VMS models considered herein depend on the stabilization parameters $\tau_m$ and $\tau_c$, which contain constants $c_1$ and $c_2$ whose value is chosen from numerical experiments. However, we can infer from (\ref{eq:static_energy}) or (\ref{eq:transfer}) how this dependency will be. As mentioned before, the last two terms in (\ref{eq:transfer}) are dissipative and therefore increasing $\tau_m$ and/or $\tau_c$ we obtain a more dissipative method. However, from (\ref{eq:tau_m})-(\ref{eq:tau_c}) increasing $\tau_m$ results in a reduction of $\tau_c$. More precisely 
%%% \begin{equation*}
%%% \tau_c=\nu + \frac{c_2}{c_1}\frac{|\a|}{h}
%%% \end{equation*}
%%% from where we see that increasing $c_1$ reduces both $\tau_m$ and $\tau_c$ but increasing $c_2$ reduces $\tau_m$ but increases $\tau_c$. On the other hand, only the fourth term in (\ref{eq:transfer}) is essential to control
%%% $ \tau_{m} \left\| \mathcal{P} \left( \mathbf{a}\cdot \mathbf{\nabla u}_{h}+\mathbf{\nabla }p_{h}\right) \right\|^{2} $, and it is possible to choose $\tau_c=0$. 
%
%%% %This is the option we followed in this work and 
%%% %In order to check the sensitivity of VMS turbulence modelling with respect to those parameters, 
%
%
%
%
%%\Subsection{Behavior in the small time step limit}
%%\label{sec:small_time_step}
%%Several references have been made in the literature to the small time step instability, the first of which in \cite{Bochev2004,Bochev2007}, and the purpose of this section is to discuss this issue. The first point to clarify is that the instability described in \cite{Bochev2007} has nothing to do with the one described in \cite{Harari2004}. Whereas the first one is attributed to consistently stabilized approximations of the Navier Stokes equations, i.e. those depending on the FE residual as VMS models, the latter is also found in the convection diffusion problem and its the manifestation of the Gibbs phenomena associated to $L^2$ projections, that dominate the discrete operator in the small time step limit.
%
%%Even though the instability described in \cite{Bochev2004,Bochev2007} was numerically observed in \cite{codina_time_2007}, the theoretical analysis in \cite{Bochev2007} is not conclusive. This is because it is shown that the semi discrete pressure operator is not uniformly bounded (it depends on h) but the right hand side is not analyzed. It is also shown, that the dissipation introduced by the time discretization scheme can be used to bound the lowest eigenvalue but introducing a restriction on the time step size. The origin of the problem is the fourth term in (\ref{eq:static_energy}).
%
%%In fact, the analysis in \cite{Burman2009} shows that stability and convergence can be proved without restrictions on the time step size provided the initial condition is appropriately chosen (as a particular projection onto a space of divergence free velocities).
%%{\color{blue} Si no entiendo mal en \cite{Burman2009} se analizan metodos de proyeccion por lo que no contradicen el analisis de \cite{Bochev2007} aunque este no sea concluyente. Dicho de otra forma, si damos por bueno el analisis de \cite{Bochev2007} tampoco hay ninguna contradiccion: los metodos basados en el residuo son inestables y los basados en proyecciones no, siempre que tomes la condicion inicial buena}.
%%The use of a non discretely divergence free initial condition is a possible explanation of the numerical results reported in \cite{Bochev2004,Bochev2007,codina_time_2007}.
%
%%In parallel, small time step instabilities were also reported in \cite{Hsu2010,gamnitzer_time-dependent_2010} although the VMS model used is different in that introduces a time step dependency on the stabilization parameter $\tau_m$. This dependency actually appears in the final discrete problem when dynamic subscales are used, as the final parameter $\tau_{m,t}$ scales with $\delta t$, as discussed in Section \ref{sec:discrete}. {\it However} this happens in conjunction with a term that comes from the discretization of a temporal derivative in (\ref{eq:velo_sgs}), see also \cite[Section 3.2]{codina_time_2007}. The plain introduction of a time step dependency in $\tau_m$ faces serious difficulties.
%%\begin{itemize}
%%\item If the convection diffusion problem is considered the method becomes unstable in the small time step limit just because the Galerkin formulation is recovered, as observed in \cite{Hsu2010}.
%%\item A similar situation occurs in the Navier-Stokes problem if $\tau_c=0$.
%%\item If $\tau_c$ is computed from (\ref{eq:tau_c}), as it is usually done, in the small time step limit $\tau_m \sim \delta t$ and $\tau_c \sim \delta t^{-1}$. If this commonly used approach is followed \cite{bazilevs_variational_2007,Hsu2010,gamnitzer_time-dependent_2010,gravemeier_algebraic_2010}, when the time step is diminished the essential numerical dissipation given by the fifth term in (\ref{eq:static_energy}) is reduced but the numerical dissipation introduced by the last term in the left hand side of (\ref{eq:static_energy}) is increased which can be a compensating effect in practice.
%%\end{itemize}
%
%%The behavior of the VMS models considered herein in the small time step limit is evaluated numerically in Section \ref{sec:TCF}.
%
%\section{Final discrete problem}
%\label{sec:discrete}
%Applying a time integration algorithm to (\ref{eq:NS_discrete})-(\ref{eq:velo_sgs})-(\ref{eq:press_sgs}) we get the fully discrete problem. The final implementation of the discrete problem is written here considering a Picard linearization of the convective term and the Backward Euler (BE) scheme for the time discretization. It can be straightforwardly modified to consider the Crank-Nicolson time integration scheme; this last scheme is the one used in the numerical examples of Sections \ref{sec:DHIT},  \ref{sec:TGV} and \ref{sec:TCF}.
%
%\subsection{Algebraic Subgrid Scales (ASGS)}
%
%Taking the nonlinear advection velocity definition (\ref{eq:a_nl}) and considering the time derivative in the fine scales, we have the Dynamic and Nonlinear ASGS method, hereinafter Dyn-Nl-ASGS. 
...
%
%In turn, $\tilde{\u}^{n,i}$ is computed by solving the discretization of the fine scale problem (\ref{eq:velo_sgs}). Note that in the nonlinear version of the algorithm, the stabilization parameter $\tau_{m,t}$ depends on the subscale itself through $\a$ in (\ref{eq:tau_m}), making the fine scale equation also nonlinear, although it is local and does not increase the size of the global linear system to be solved. At each integration point of each element we iteratively solve
%\begin{equation}
%\label{eq:velo_sgs_discrete_expl}
...
%% \end{equation}
%
%%We can observe in equation (\ref{eq:velo_sgs_discrete_expl}) that the fine scale velocity $\tilde{\u}$ is also nonlinear since the stabilization parameter $\tau_{m,t}$ and the advection velocity $\a$ depend on $\tilde{\u}$.
%% TILL HERE
...
%\begin{align}
%\label{eq:velo_sgs_discrete_impl}
%\tilde{\mathbf{u}}^{n,i,k}
%& + \tilde{\u}^{n,i,k}\cdot\nabla\u_h^{n,i}
...
%
%For the simplest ASGS scheme we do not consider the time derivative of the fine scale, we consider them quasi-static, i.e., $(\partial_t\tilde{\u},\v_h)=0$. Note that in any case the subscales will depend on time through the FE residual and the stabilization parameter. On the other hand, the advection velocity is considered to be linear as indicated in (\ref{eq:a_lin}). We label this method as Static Linear ASGS (Sta-Lin-ASGS). Note that the Sta-Lin-ASGS method does not need to explicitly compute $\tilde{\u}$; invoking (\ref{eq:tau_m}) and (\ref{eq:tau_c}) in (\ref{eq:NS_discrete}) we get a discrete equation only in terms of the FE component.%, avoiding to solve (\ref{eq:velo_sgs_discrete_impl}).
%
...
%
%Let us state the Dynamic and Nonlinear OSS (Dyn-Nl-OSS) method, which means to take into account the nonlinearity of the advection velocity (\ref{eq:a_nl}) and the time derivative of the subscales. At time step $n$ and nonlinear iteration $i$, given $\u_h^{n,i-1}$, $\u_h^{n-1}$, $\tilde{\u}^{n,i-1}$ and $\tilde{\u}^{n-1}$ we compute $\mathbf{u}_h^{n,i}$ and $ p_h^{n,i} $ by solving
...
%
%Like the Dyn-NL-ASGS method, we also need to compute the subscale velocity $\tilde{\u}$ explicitly. We compute the discrete subscale problem with the OSS counterpart of (\ref{eq:velo_sgs_discrete_expl}) or (\ref{eq:velo_sgs_discrete_impl}), viz. 
%% \begin{align}
%% \label{eq:velo_sgs_discrete_impl_OSS}
...
%%& + \tau_{m,t}^{k-1} \left[\mathbf{f}-\left(\frac{(\u_h^{n+1,i}-\u_h^n)}{\Delta t}+N(\u_h^{n+1,i},\u_h^{n+1,i})+\nabla p_h^{n+1}\right)\right].
%\end{align}
%Note that $\boldsymbol{\xi}_m^{n,i}$ actually depends on $\tilde{\u}^{n,i}$ via the advection velocity of the convective term $\a^{n,i}=\u_h^{n,i}+\tilde{\u}^{n,i}$. In order to simplify the fine scale computation (\ref{eq:velo_sgs_discrete_impl_OSS}) we use the projection at the previous nonlinear iteration, i.e., $\boldsymbol{\xi}_m^{n,i-1}$. 
%
...
%%&(\tau_c\xi_c^{n,i},q_h)=(\tau_c R_p^{n,i},q_h)&&\forall q_h\in\mathcal{Q}_{0,h}.
%%\end{align}
%where the residuals $\R_u^{n,i}$ and $R_p^{n,i}$ are evaluated using (\ref{eq:Ru}) and (\ref{eq:Rp}) with $\a^{n,i}=\u_h^{n,i}+\tilde{\u}^{n,i}$, $\u_h^{n,i}$, $\u_h^{n-1}$ and $p_h^{n,i}$. %It is also possible to compute projections explicitly, using the residuals evaluated with $\a^{n-1}=\u_h^{n-1}+\tilde{\u}^{n-1}$, $\u_h^{n-1}$, $\u_h^{n-2}$ and $p_h^{n-1}$ but we do not use this option here.  
% Note that when convergence of the nonlinear iteration is achieved, (\ref{eq:velo_sgs_discrete_impl_OSS}) and (\ref{eq:dyn_proj}) guarantee that $(\v_h,\tilde{\mathbf{u}}^{n}) = 0$ for any $\v_h\in\mathcal{V}_{0,h}$.
%
%%In the following numerical experiments, we use the algorithmic constants $c_1=12$ and $c_2=8$ for $\tau_m$ and we set $\tau_c=0$. A detailed justification of this choice is provided in Section \ref{sec:effect_const}.
%
%\section{Decay of homogeneous isotropic turbulence}
%\label{sec:DHIT}
%\subsection{Problem definition}
%This problem, one of the most used benchmarks to test LES models, consists of analyzing the statistics of the turbulent flow in a 3D box of size $\Omega=(0,2\pi)\times(0,2\pi)\times(0,2\pi)$ with periodic boundary conditions in all directions, which is started with a field having a predetermined energy spectrum. A detailed description of the computational domain, initial conditions, and problem setting is given in \cite{mansour_decay_1994}, %{\color{blue} 
%which we follow here. A description on the influence of the domain size on the results is given in \cite{ishida_decay_2006}.%} 
%We solve this problem using the different VMS models of Section \ref{sec:discrete} and structured meshes with $N_x\times N_y\times N_z$ linear, quadratic and cubic hexahedral elements, taking $(N_x,N_y,N_z)= (32,32,32),\,(64,64,64)$ and $(128,128,128)$.
%
%\subsubsection{Initial condition}
%
%The initial energy spectra \cite{mansour_decay_1994} is given by
%\begin{equation}
%\label{eq:ini_E_DHIT1}
%E(k,0)=\frac{q^2}{2A}k_0^{-(\sigma+1)}k^4\exp\left(-\frac{\sigma}{2}\left(\frac{k}{k_0}\right)^2\right),
%\end{equation}
%where $k_0$ is the wave number at which $E(k,0)$ is maximum, $q^2/2=3/2$ is the total kinetic energy, $\sigma=4$ and $A=\int_0^\infty k^\sigma\exp(-\sigma k^2/2)=\frac{3}{32}\sqrt{\frac{\pi}{2}}$.  
%%In this case we choose $q^2=3$ and $\sigma=4$, which supposes that $A=\frac{3}{32}\sqrt{\frac{\pi}{2}}$.
%%With this setting, it can be shown that $A=\frac{3}{32}\sqrt{\frac{\pi}{2}}$. So, the initial energy spectra will be:
%%\begin{equation}
%%\label{eq:ini_E_DHIT2}
%%E(k,0)=16\sqrt{\frac{2}{\pi}}k_0^{-5}k^4\exp\left(-2\left(\frac{k}{k_0}\right)^2\right).
%%\end{equation}
%Following Rogallo \cite{rogallo_numerical_1981}, we generate the initial field on the Fourier space such that it satisfies continuity and has the energy spectrum prescribed in (\ref{eq:ini_E_DHIT1}). 
%% First, we define two complex parameters:
%% \begin{align}
%% \label{eq:alpha_E0_DHIT}
%% &\alpha=\left(\frac{E(k,0)}{2\pi k^2}\right)^{1/2}\exp(i\theta_1)\cos(\phi),\\
%% \label{eq:beta_E0_DHIT}
...
%% \begin{align}
%% \label{eq:ini_FFTu_DHIT}
%% &\hat{u}_1=(\alpha|k|k_2+\beta k_1k_3)/(|k|k_h),\\\nonumber
%% &\hat{u}_2=(-\alpha|k|k_2+\beta k_3k_2)/(|k|k_h),\\\nonumber
%% &\hat{u}_3=(-\beta k_h^2)/(|k|k_h),
%% \end{align}
%% where $k_h=\left(k_1^2+k_2^2\right)^{1/2}$ and $|k|=\left(k_1^2+k_2^2+k_3^2\right)^{1/2}$.
%% To have the field (\ref{eq:ini_FFTu_DHIT}) in the physical space we apply the inverse Fast Fourier Transform.
%We finally transform the initial field to the physical space applying the inverse Fast Fourier Transform.
%We compare the results against those available in \cite{_selection_????}, so we set $k_0=6$ and the viscosity such that the associated Taylor-microscale Reynolds number is ${\rm Re}_\lambda=952$,
...
%which results in $\nu=3.5014006\cdot 10^{-4}$. 
%%The initial analytical energy spectrum and the one computed in a $128^3$ $Q$1 elements mesh are shown in Fig.~\ref{Fig:ini_spec_DHIT}
%
%% In Figure \ref{Fig:ini_spec_DHIT} we show the initial analytical energy spectrum and the one calculated from the initial velocity field on the Fourier space (\ref{eq:ini_FFTu_DHIT}) using a $128^3$ linear elements mesh. We observe that the computed energy spectra fits to the analytical one. It is what we expect since we use the initial field proposed by Rogallo \cite{rogallo_numerical_1981}, which precisely satisfies this condition.
%
%% \begin{figure}[h!]
%% 	\centering	
%% 	\includegraphics[width=0.5\textwidth]{Figures/DHIT/Initial_E}
%% 	\caption{Analytical and computed initial energy spectra}
%% 	\label{Fig:ini_spec_DHIT}
%% \end{figure}
%
%\subsubsection{Setting}
%\label{subsubsec:DHIT_setting}
%%As said before, we solve the DHIT test using different meshes ($32^3$, $64^3$ and $128^3$ elements). 
%%The initial energy spectrum is the defined in (\ref{eq:ini_E_DHIT2}) with a peak on the wavenumber $k_0=6$ which supposes that the total kinetic energy at the initial time step should be $1/2q^2=1.5$ and the enstrophy $\frac{1}{2}<w^2>=67.5$.
%
%%In what concerns on the tested VMS methods, we use 
%We test the ASGS method and the OSS method reported in Section \ref{sec:discrete}. The problem is solved considering three different cases for both methods, depending on the definition of the advection velocity and the tracking of the subscales. The advection velocity $\a$ can be linear or nonlinear and the subscales can be dynamic or static (see Subsection \ref{subsec:VMS_framework}). Table \ref{table:DHIT_cases} collects all the VMS combinations to be compared for the different simulations.
%
%\begin{table}[h]
%\centering
%\begin{tabular}{ccccc}
...
%\end{table}
%
%In terms of the numerical parameters of the methods, we use the skew-symmetric convective term \textit{type 1} defined in Subsection \ref{subsec:NS_formulation}. 
%The stabilization parameter $\tau_c$ is set equal to zero and the algorithmic constants in $\tau_m$ are $c_1=12$ and $c_2=2$ (see Section \ref{sec:effect_const}).
%%according what we have exposed in Subsection \ref{subsec:sensivity_tau}. 
...
%At each time step the nonlinear system is solved as described in Section \ref{sec:discrete}. 
%
%\subsection{Numerical results}
%\label{subsec:result_DHIT}
%\subsubsection{Energy Conservation}
%\label{subsubsec:ene_cons_DHIT}
%% In order to check the conservation of energy of the problem and to ensure that the arguments developed at section \ref{sec:energy} are correct, we evaluate each term for the DHIT test. Further, this type of test is excellent as a way to check the correct implementation of the method. 
...
%\begin{figure}[h!]
%  \centering
%  \subfigure[Mean flow equation energy balance]{\label{fig:OSS_MF_balance}\includegraphics[width=0.49\textwidth]{Figures/DHIT/MF_ene_balance_OSS_dyn_nl}}    
%  \subfigure[Subscale equation energy balance]{\label{fig:OSS_ss_balance}\includegraphics[width=0.49\textwidth]{Figures/DHIT/SS_ene_balance_OSS_dyn_nl}}
%  \caption{Mean flow and subscale energy balances the Dyn-Nl-OSS method.}
%  \label{fig:OSS_ene_balance_split}
%\end{figure}
%
%%Now we show the 
%The global energy balance terms obtained solving the problem with the skew-symmetric convective term \textit{type 2} for the Dyn-Nl-ASGS and Dyn-Nl-OSS cases are shown in Fig. \ref{fig:ene_balance_sk2}. We note that the loss of skew-symmetry in the convective term has a non-negligible effect (see Figs.~\ref{fig:ASGS_ene_balance2} and \ref{fig:OSS_ene_balance2}). In particular, this term introduces negative dissipation (production of energy) into the problem. %As we will show in Section \ref{subsubsec:Total_ene_DHIT}, 
%This fact implies that the method is less dissipative and the energy decays at a slower rate than using the convective term \textit{type 1} and the method seems to be less diffusive. This negative contribution, however, is not predictable and could result in a blow up of the calculation. We refer to Section \ref{sec:effect_const} for further comments about numerical instabilities associated to the \textit{type 2} convective term.%Although no theoretical energy estimates are available we have not observed instabilities in practice.
%
%\begin{figure}[h!]
%	\centering	
%	\subfigure[Global energy balance for Dyn-Nl-ASGS]{\label{fig:ASGS_ene_balance2}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_balance_ASGS_sk2}}
%	\subfigure[Global energy balance for Dyn-Nl-OSS]{\label{fig:OSS_ene_balance2}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_balance_OSS_sk2}}
...
%
%\label{subsubsec:comp_cost_DHIT}
...
%
%An important issue when comparing different computational methods is their corresponding computational cost. In order to characterize the performance of the different VMS methods introduced in Section \ref{sec:formulation},  we analyze  some quantities that define the computational cost of each method, viz. nonlinear iterations, iterative solver iterations, and the adaptive time step evolution.
%
...
%	%\subfigure[Accumulated solver iterations at each time step for]{\label{fig:Acsoliter}\includegraphics[width=0.49\textwidth]{Figures/DHIT/Acsoliter_32_scaled_cnvgd}}
%	%\caption{Maximum and accumulated solver iterations}
%	\subfigure[$32^3$ mesh]{\label{fig:soliter32}\includegraphics[width=0.49\textwidth]{Figures/DHIT/Acsoliter_32_scaled_cnvgd}}
%	\subfigure[$64^3$ mesh]{\label{fig:soliter64}\includegraphics[width=0.49\textwidth]{Figures/DHIT/Acsoliter_64}}
...
%%}
%
%%An interesting and important consideration arising from the adaptive time step that needs to be noted is that, if we define the reduction and amplification factors in a such way that there is a big difference between the two, the steps after a reduction are closer, which can be observed on the density of the points in Fig. (\ref{fig:Acsoliter}) after a reduction. Then, with this definition we are ensuring that the steps after a non-converged computation, which are supposed to be also tricky to solve, use a small enough time step. But, as a counterpart, the global computation steps are significantly increased.
%
%%Summarizing,   ASGS  methods need less nonlinear iterations, but on the other hand  OSS  methods need less solver iterations. On the other hand, ASGS formulations are prone to instabilities; linear formulations diverge and the nonlinear dynamic formulation requires drastical time step reductions in order to attain convergence. In order to combine all these aspects and provide a global computational cost estimate, we plot the accumulated linear solver iterations (in nonlinear and time loops) in Fig. \ref{fig:Acsoliter}.% for the converged cases. 
%%Fig. \ref{fig:Acsoliter} is an explanatory figure. In there we can see that the reduction on solver iterations has more effect than the reduction on the nonlinear iterations for the DHIT test. 
%
%% We can clearly state that the most efficient method for this setting, in terms of computational cost, is the dynamic and linear OSS method; all OSS cases are below ASGS. It has to be said that the dynamic nonlinear OSS case has also a good performance. Excluding the fact that it needs two time step reductions around $t=0.6$ and $t=1.1$ , we see that its slope is the lowest one. This happens because it needs less solver iterations and also because it requires less nonlinear iteration than the Dyn-Lin-OSS case; the method would probably be faster than Dyn-Lin-OSS  for larger time step amplification factors.
%
%\subsubsection{Total energy evolution}
%\label{subsubsec:Total_ene_DHIT}
%In this section we present the total energy evolution of the resolved scales, i.e., the FE component. %It can be seen that for a coarse mesh like the $32^3$ linear hexaedral elements mesh that we are using for this test, the total amount of energy at the initial time step (computed after the Inverse Fast Fourier Transform of the velocity field is far from the theoretical starting point $E_0=1.5$. This error is due to the proijection from the Fourier space onto the FE space, which obviously depends on the mesh size. As we refine the mesh, the initial value of total energy tends to $1.5$, as expected. Knowing this behavior, it is difficult to draw conclusions comparing the simulations with DNS results \cite{_selection_????} and therefore  we scale the initial condition in a such way that its initial energy achieve the desired value. 
...
%	% \subfigure[Using the skew-symmetric term \textit{type 2}]{\label{fig:total_ene_32_sk2}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_skew_scaled}}
%	% \caption{Total energy evolution for a $32^3$ elements mesh with the scaled initial condition}
%	% \label{fig:total_ene_32}
%	\subfigure[$32^3$ mesh]{\label{fig:total_ene_32}\includegraphics[width=0.49\textwidth]{Figures/DHIT/total_ene_32_scaled_loglog.pdf}}
%	\subfigure[$64^3$ mesh]{\label{fig:total_ene_64}\includegraphics[width=0.49\textwidth]{Figures/DHIT/total_ene_64_scaled_loglog.pdf}}
%	\caption{Total energy evolution for the $32^3$ and $64^3$ elements meshes with the scaled initial condition.}
%	\label{fig:total_ene}
%\end{figure}
%
%%The results shown so far have been obtained using the skew-symmetric convective term \textit{type 1} defined in (\ref{eq:b_skew1}). 
%%But what happens if we choose another definition for this term? We know, by some experience acquired on other tests not reported in this work, e.g. Destabilization of an ordered initial condition test (2D and 3D), that the nonskew-symmetric term (\ref{eq:b_noskew}) can involve convergence problems. So 
%%We test the influence of this term by solving the same problem with the convective skew-symmetric term (\textit{type 2)}  in (\ref{eq:b_skew2}) for dynamic and nonlinear ASGS and OSS schemes. The results obtained for this test are shown in Fig. \ref{fig:total_ene_32_sk2}, where it is seen that the solution using the \textit{type 2} term is above the one obtained with term \textit{type 1}, which shows that the convective term is actually productive.
%%. Then, we can say that the solution is less dissipative when we use this convective term which agree with the results shown in the energy conservation analysis done in subsection \ref{subsubsec:ene_cons_DHIT}. This result is a consequence of the fact that the convective term is non zero as it is exposed on remark \ref{rem:skewsym}, equation (\ref{eq:mkgkdfsnmgks}).
%
...
%\begin{figure}[h!]
%  \centering
%  \subfigure[$32^3$ mesh]{\label{fig:spec_32}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_32_02_scaled}}    
%  \subfigure[$64^3$ mesh]{\label{fig:spec_64}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_64_08}}
...
%  \subfigure[Total evolution]{\label{fig:ene_hp_global}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_hp_scaled_loglog}}    
%  \subfigure[Close up view]{\label{fig:ene_hp_close}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_hp_close_scaled_loglog}}
%  \caption{Total kinetic energy evolution.}
%  \label{fig:ene_hp}
%\end{figure}
%
...
%  \subfigure[Energy spectra at $t=0.8$]{\label{fig:spec_hp_08}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_hp_08}}
%  \subfigure[Energy spectra at $t=1.0$]{\label{fig:spec_hp_1}\includegraphics[width=0.5\textwidth]{Figures/DHIT/spec_hp_1}}
...
%% \begin{figure}[h!]
%%   \centering
%%   \subfigure[Energy evolution]{\label{fig:ene_q2q1}\includegraphics[width=0.49\textwidth]{Figures/DHIT/ene_q2q1_scaled}}  
%%   \subfigure[Energy spectra at $t=0.2$]{\label{fig:spec_q2q1_02}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_q2q1_scaled_t02}}\\  
%%   \subfigure[Energy spectra at $t=0.4$]{\label{fig:spec_q2q1_04}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_q2q1_scaled_t04}}
%%   \subfigure[Energy spectra at $t=0.8$]{\label{fig:spec_q2q1_08}\includegraphics[width=0.49\textwidth]{Figures/DHIT/spec_q2q1_scaled_t08}}
...
%\subsection{Problem definition}
%
%The Taylor-Green vortex (TGV) problem is a typical and widely used problem in turbulence numerical simulations. 
%%First introduced by Taylor and Green (1937) {\color{red}(es necesario citar esto?)}, 
%This problem aims to show, in a relatively simple flow, the basic turbulence decay mechanisms like the turbulent energy cascade, the production of small eddies and the enhancement of dissipation by the stretching of vortex lines.
%
%\subsubsection{Initial condition}
%
%As in the previous example, the computational domain is the unit cube with periodical boundary conditions. The initial analytical condition for this problem, unlike the DHIT problem, is defined in the physical space (see, e.g., \cite{gassner_accuracy_????}), and given by
%\begin{align}
%\label{eq:ini_sol_TG}
...
%We choose $\gamma=0$, which gives the mean initial velocity  $u_0=1$.
%The initial velocity field on the Fourier space has eight modes located at the wave numbers $\mathbf{k}=(\pm1,\pm1,\pm1)$, which means that the initial flow generates a single vortex scale. The pressure constant parameter $p_0$ is chosen equal to zero.
% 
%%The Reynolds number is defined as the inverse of the kinematic viscosity $\nu$, noting that the length and velocity scales are of the same order. This is done according to \cite{gassner_accuracy_????} and \cite{brachet_direct_1991}, and will allow us to compare our results with those showed on these papers. For simplicity, the pressure constant parameter $p_0$ is chosen equal to zero.
%
%%An interesting result coming from the definition of the initial condition (\ref{eq:ini_sol_TG}) is its representation on the Fourier space. According to Fauconnier et al, \cite{fauconnier_construction_2009}, the initial velocity field on the Fourier space corresponds to eight Fourier modes located at $\mathbf{k}=(\pm1,\pm1,\pm1)$. It means that the initial flow generates a single vortex scale, which will be shown on the following results.
%
%\subsubsection{Setting}
%
...
%We use the same VMS methods as for the DHIT problem defined in Section \ref{sec:DHIT} to solve this test, namely the ASGS and OSS methods, both with linear and nonlinear definitions of the convective term and static or dynamic tracking in time of the subscales, as it is summarized in Table \ref{table:DHIT_cases}.
%The stabilization parameters for each method are the same as those chosen for the DHIT test, see Subsection~\ref{subsubsec:DHIT_setting}, and discussed in Section \ref{sec:effect_const}.
%
%Initially we consider a mesh of $32^3$ hexahedral linear elements $(Q1)$, but we will redefine this discretization to analyze the method performance when we refine the mesh, decreasing the element size $h$ or increasing the degree of the interpolation polynomial $p$. It implies to solve the problem on meshes with $64^3$ and $128^3$ linear $(Q1)$, quadratic $(Q2)$ or cubic $(Q3)$ hexahedral elements. We also use a $20^3(Q3)$ discretization to compare against other authors results.
%
%\subsection{Numerical results}
...
%\begin{figure}[h!]
%  \centering
%  \subfigure[Isosurface for $|\omega|=1.0$ at $t=0.0$]{\label{fig:isvor_vel_1}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_1}}    
%  \subfigure[Isosurface for $|\omega|=1.0$ at $t=2.0$]{\label{fig:isvor_vel_2}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_2}}\\
%    \centering
%  \subfigure[Isosurface for $|\omega|=2.5$ at $t=4.1$]{\label{fig:isvor_vel_3}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_3}}    
%  \subfigure[Isosurface for $|\omega|=5.0$ at $t=6.1$]{\label{fig:isvor_vel_4}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_4}}\\
%    \centering
%  \subfigure[Isosurface for $|\omega|=8.0$ at $t=8.2$]{\label{fig:isvor_vel_5}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_5}}    
%  \subfigure[Isosurface for $|\omega|=9.0$ at $t=10.2$]{\label{fig:isvor_vel_6}\includegraphics[clip=true, trim=13cm 0cm 13cm 0cm, width=0.45\textwidth]{Figures/TG/isovorti_veloc_6}}\\
...
%As for the DHIT test, the results obtained using the different methods listed in Table \ref{table:DHIT_cases} are very similar 
%for these coarser discretizations. The only point that is worth to note is that the linear and static ASGS case (Sta-Lin-ASGS) and the dynamic and linear ASGS case (Dyn-Lin-ASGS) diverge at some time step before $t=9$. Anyway, all the methods converge as $h\rightarrow0$ and the accuracy depends much more on the mesh size than on the choice of the method. In turn, similar trends for the computational cost analyzed in the previous section have been observed.
%
%% We want to show the amount of numerical dissipation, 
%% the energy cascade in the spectra and the enstrophy evolution (compared to DNS) in all cases. We compare first the kinetic energy evolution  with the kinetic energy evolution obtained by Brachet \emph{et al.} \cite{brachet_small-scale_1983}, (Fig. \ref{fig:ene_32_TG}). We also present the energy spectra at $t=9$, when the flow is supposed to be nearly isotropic at large wave numbers, (Fig. \ref{fig:spec_32_TG}).
%
%%  \begin{figure}[h!]
%% 	\centering	
%% 	\subfigure[Total kinetic energy evolution]{\label{fig:ene_32_TG}\includegraphics[width=0.4\textwidth]{Figures/TG/ene_32_new}}
%% 	\subfigure[Energy spectra at $t=9.0$]{\label{fig:spec_32_TG}\includegraphics[width=0.4\textwidth]{Figures/TG/spec_32_x2_new}}
%% 	\caption{Total kinetic energy evolution and energy spectra}
%% 	\label{fig:ene_spec_32_TG}
%% \end{figure}
%
%% In Fig. \ref{fig:ene_32_TG}, we can see that for a $32^3$ trilinear hexahedral elements mesh all methods show a premature decay of energy. We recognize the same behavior observed for the same mesh in the DHIT test, see Subsection \ref{subsubsec:Total_ene_DHIT}. For this mesh, it is clear that the methods are not able to simulate properly the transition to turbulence. The energy spectra at $t=9.0$ shows us that the flow is isotropic at large wave numbers since it is decaying following the $k^{-5/3}$ Kolmogorov law. 
%
...
% \begin{figure}[h!]
% 	\centering
%	\subfigure[Total kinetic energy evolution]{\label{fig:ene_hp_TG}\includegraphics[width=0.49\textwidth]{Figures/TG/ene_hp_10_new}}
%	\subfigure[Energy spectra at $t=9$]{\label{fig:spec_hp_TG}\includegraphics[width=0.49\textwidth]{Figures/TG/spec_hp_x2_new}}
...
%\begin{figure}[h!]
%	\centering	
%	\subfigure[Resolved]{\label{fig:enediss_hp_TG_resolved}\includegraphics[width=0.49\textwidth]{Figures/TG/ens_hp_10_new_resolved}}
%	\subfigure[Total]{\label{fig:enediss_hp_TG_total}\includegraphics[width=0.49\textwidth]{Figures/TG/ens_hp_10_new_total}}
...
%\begin{figure}[h!]
%	\centering	
%	\subfigure[Resolved]{\label{fig:enediss_dynsmag_resolved}\includegraphics[width=0.49\textwidth]{Figures/TG/ens_64dofs_dynsmag_resolved}}
%	\subfigure[Total]{\label{fig:enediss_dynsmag_total}\includegraphics[width=0.49\textwidth]{Figures/TG/ens_64dofs_dynsmag_total}}
...
%\label{sec:TCF}
%%In all the tests shown so far (sections \ref{sec:DHIT} and \ref{sec:TGV}), we have solved homogeneous and free-evolving turbulent problems. But we also are interested on other type of turbulent problems in order to check the behavior of the defined VMS methods. According to \cite{pope_turbulent_2000}, most turbulent flows are bounded by one or more solid surfaces, giving wall-bounded flows. There are several turbulent wall-bounded tests, e.g. fully developed channel flow, fully developed pipe flow and the flat-plate boundary layer. We test just one of them, in particular the fully developed channel flow.
%After studying the performance of VMS in the LES of homogeneous flows we turn our attention to wall-bounded turbulent flow and present results of %the simplest among these kind of problems, 
%fully developed turbulent flow in a channel.
%%The fully developed channel flow is a widely used wall-bounded turbulent test. 
%%In this section we present results of the numerical simulation of the turbulent flow in a channel, the simplest 
%
%\subsection{Problem definition}
%%An evidence of this is the large amount of experimental and computational works done about it. This test consists of a flux that flows between two parallel walls, where the mean velocity vector is parallel to the wall. The most important issues to consider when characterising the flow are the mean velocity profiles, velocity fluctuations and the shear stress carried by the fluid on the wall. It has to be noticed that we will concern only on the fully developed flow, which means that the turbulent regime has been reached at the time when we start our flow characterization.
%
%%In this test, like in most of turbulent flow tests, the Reynolds number has an important role when defining the test parameters. In particular, in the turbulent channel flow test, the most characteristic parameter is the Reynolds number based on the wall shear velocity, $Re_\tau$. In the literature, there appear basically three different values of this parameters: $Re_\tau=590$, $Re_\tau=395$  and $Re_\tau=180$, see \cite{bazilevs_variational_2007, Calderer2013, gamnitzer_time-dependent_2010, gravemeier_algebraic_2010, gullbrand_effect_2003, hughes_large_2001, john_variants_2008, kim_turbulence_1987, masud_variational_2011, moser_direct_1999}. We will restrict our work to $Re_\tau=180$ and $Re_\tau=395$ cases.
%
%This test consists of a fluid that flows between two parallel walls driven by an imposed pressure gradient which is defined by the Reynolds number based on the wall shear velocity, ${\rm Re}_\tau$. In the important amount of literature devoted to this problem, the usual Reynolds numbers are: ${\rm Re}_\tau=590$, ${\rm Re}_\tau=395$  and ${\rm Re}_\tau=180$ (see \cite{bazilevs_variational_2007, Calderer2013, gamnitzer_time-dependent_2010, gravemeier_algebraic_2010, gullbrand_effect_2003, hughes_large_2001, john_variants_2008, kim_turbulence_1987, masud_variational_2011, moser_direct_1999}). We will restrict our attention to ${\rm Re}_\tau=180$ and ${\rm Re}_\tau=395$.
%
%\subsubsection{Computational domain and mesh definition}
%
...
%\begin{figure}[h!]
%	\centering	
%	\includegraphics[width=0.5\textwidth]{Figures/CF/Chanel_geom}
%	\caption{Channel computational domain.}
%	\label{fig:Channel_geo}
%\end{figure}
%
%In the wall-normal direction boundaries ($y=-\delta$ and $y=\delta$) we impose a non-slip condition. The streamwise and spanwise directions are assumed to be homogeneous, so we use periodic boundary conditions in these directions.\footnote{According to Kim  \emph{et al.} \cite{kim_turbulence_1987}, the use of periodic boundary conditions in the homogeneous directions can be justified when the computational box is such that the largest eddies in the flow fit in the computational domain. This is the case considered here.}
%%In \cite{kim_turbulence_1987} is shown the adequacy of this computational domain for a Reynolds number based on the wall shear velocity $Re_\tau=180$.
%
%%We solve the problem using different mesh discretizations, all of them uniform in the homogeneous directions and refined near the walls in the wall-normal direction. In particular we will use meshes composed by $32^3$, $64^3$ or $128^3$ hexahedral linear ($Q1$), quadratic ($Q2$) or cubic ($Q3$) elements. 
...
%f_x=\frac{\tau_w}{\delta}.$$ 
%
%% In order to compare our results with the DNS computed in \cite{moser_direct_1999}, which is a continuation of the work done in \cite{kim_turbulence_1987}, we use the same parameters value defined on these articles. The main turbulent properties defined on the cited works from which we have based our computations are listed below.
%% \begin{center}
%% \begin{tabular}{|c|c|c|}
%% \hline
%% $Re_\tau$&$180$&$395$\\
...
%% \end{tabular}
%% \end{center}
%% With $Re$ the Reynolds number based on the bulk velocity, $Re:=\frac{2\delta \bar{U}}{\nu}$, and $Re_0$ the Reynolds number based on centerline velocity, $Re_0:=\frac{U_0\delta}{\nu}$.
%
%% These values allow us to define the initial condition, but we can not impose them to keep constant along the time. Then, after the computation we will obtain the computed values which in a statistically stable regime must be similar to them.
%
%% We define the bulk mean velocity and the half channel height equal to one, $\bar{U}=1$ and $\delta=1$. From the Reynolds number based on the bulk mean velocity, $Re$, we obtain the viscosity as
%% $$\nu=\frac{2\delta\bar{U}}{Re}=\frac{2}{Re}.$$
%
%% Knowing the viscosity and the friction Reynolds number, we obtain the friction velocity and the wall shear stress,
%% $$u_\tau=\frac{\nu Re_\tau}{\delta}\quad\quad\mbox{and}\quad\quad\tau_w=\rho u_\tau^2.$$
%
%% From the wall shear stress we can determine the value of the driving force. We impose a driving force equivalent to the mean axial pressure gradient, then
%% $$f_x=-\frac{dp_w}{dx}=\frac{\tau_w}{\delta}.$$ 
%
...
%
%
%Our purpose is to check the VMS methods defined in Subsection \ref{subsec:VMS_framework} for a wall-bounded flow. Following the computations performed for the previous tests, we solve the problem using the same cases defined in Table \ref{table:DHIT_cases} and the numerical parameters $\tau_c=0$ and $\tau_m$ are defined in the same way, now with the algorithmic constants  $c_1=12$ and $c_2=8$ (see Subsection \ref{sec:effect_const}) and the characteristic length, $h$, is chosen to be the minimum element length. 
%%However, in the definition of the stabilization parameters (\ref{eq:tau_m})-(\ref{eq:tau_c}), the characteristic length, $h$, is choosen to be the minumum element length.
%
%\subsection{Numerical results}
...
%
%%To clarify notation, hereinafter the symbol $\langle\cdot\rangle$ will denote the mean value in streamwise and spanwise direction, but also in time. 
%%That is, for a certain quantity, $q(x,y,z,t)$, its mean value depending on the wall-normal direction, $y$, can be calculated as
%%$$\langle q\rangle(y)=\frac{1}{L_xL_zT}\int_0^{L_x}\int_0^{L_z}\int_0^Tq(x,y,z,t)\ dxdzdt,\quad\quad\mbox{for $x\in[0,L_x]$, $z\in[0,L_z]$ and $t\in[0,T]$}.$$
%%
%%For a discrete variable, $q_h(i,j,k,n)$ with $i=1,...,N_x$, $j=1,...,N_y$, $k=1,...,N_z$ and $n=1,...,N_t$; its mean value is computed adding its periodic components and dividing by the number of points, which reads
%%$$\langle q_h\rangle(j)=\frac{1}{N_xN_zN_t}\sum_{i=1}^{N_x}\sum_{k=1}^{N_z}\sum_{n=1}^{N_t}q_h(i,j,k,n)\quad\quad\forall j=1,...,N_y,$$
%%where $N_x$, $N_y$, $N_z$ and $N_t$ are the total amount of discrete points in streamwise direction, wall-normal direction, spanwise direction and time, respectively.
%
...
%\begin{figure}[h!]
%	\centering	
%	\subfigure[Mean streamwise velocity]{\label{fig:Channel_umean_32}\includegraphics[width=0.49\textwidth]{Figures/CF/umean}}
...
%\end{figure}
%%Other common quantities used to characterize the turbulent channel flow are the root-mean-square velocity fluctuations. %Here, the rms velocity fluctuation, $q'(y)$, of certain quantity, $q(x,y,z,t)$, is computed as follows 
%%\begin{align*}
%%q'(y)&=\left\langle\left(q(x,y,z,t)-\langle q\rangle(y)\right)^2\right\rangle^{1/2}=\left(\left\langle q(x,y,z,t)^2\right\rangle-\left\langle 2q(x,y,z,t)\langle q\rangle(y)\right\rangle+\left\langle\langle q\rangle(y)^2\right\rangle\right)^{1/2}=\\
%%&=\left(\langle q(x,y,z,t)^2\rangle-\langle q\rangle(y)^2\right)^{1/2}.
%%\end{align*}
%%
% %The results obtained for all the cases considered are also compared against the DNS data \cite{moser_direct_1999}.
%
...
%
%It can be seen that for the discrete equation (\ref{eq:NS_discrete}), one can obtain the Reynolds shear stress defined as follows:
%\begin{equation}
%\label{eq:Rey_shear_disc}
...
%$ \tau_{m} \left\| \mathcal{P} \left( \mathbf{a}\cdot \mathbf{\nabla u}_{h}+\mathbf{\nabla }p_{h}\right) \right\|^{2} $ and it is possible to choose $\tau_c=0$. 
%
%%All the results presented above have been obtained using the same setting of the numerical stabilization parameters $\tau_m$ and $\tau_c$. In all tests, we take the algorithmic constants $c_1=12$ and $c_2=8$ for $\tau_m$ and we set $\tau_c=0$.  
%The results presented above have been obtained using different settings of the numerical stabilization parameters $\tau_m$ and $\tau_c$. In DHIT and TGV tests, we take the algorithmic constants $c_1=12$ and $c_2=2$ for $\tau_m$ and we set $\tau_c=0$, while for the TCF test we have used $c_1=12$ and $c_2=8$ for $\tau_m$ and also $\tau_c=0$. In this section we analyze the influence of these parameters on the numerical results and justify our choice of the constants for the large eddy simulation of turbulent flows.%\footnote{We also refer to  (Matias 2013), where a comparison between choosing $c_1=4$ and $c_1=12$ is done for the Turbulent Channel Flow test using dynamic and nonlinear ASGS method, concluding that a less dissipative scheme is obtained setting $c_1=12$. This result follows the argumentation above, where we state that the value of $c_1$ has more influence in $\tau_c$ than in $\tau_m$ for high Reynolds problems.
%
%%These values have been chosen according a previous analysis based on the computational cost and the numerical dissipation introduced into the problem for the DHIT test (see section \ref{subsec:sensivity_tau}). 
%
%%The selection of these values can be done from an analytical point of view, as it is done from a Fourier analysis in \cite{codina_stabilized_2000}, where is stated that the algorithmic constants may be taken as $c_1=4$ and $c_2=2$ for a 1D scalar convection-diffusion-reaction problem in the advective limit.
%
%We have performed a sensitivity analysis of the VMS schemes with respect to the value of $c_1$ and $c_2$. To see the effect of such algorithmic constants on $\tau_m$ and $\tau_c$ independently, we define a new constant $c_c$ which allows us to redefine (\ref{eq:tau_c_new}) as
%\begin{equation}
%\label{eq:tau_c_cc_new}
%\tau_c=c_c\left(\nu + \frac{c_2}{c_1}{|\a|}{h}\right)
%\end{equation}
%%These experiments have been done with the DHIT test using the Dyn-Nl-OSS case in a $32^3$ $Q$1 mesh and are depicted in Figure \ref{fig:tau_analysis}. They show some changes in the behavior of the VMS methods in terms of energy dissipation. In particular, for high Reynolds number problems, the constant $c_1$ does not have so much influence on $\tau_m$, but it does on $\tau_c$ (when $c_c$ is defined as in (\ref{eq:tau_c}), $c_c=1/c_1$). We observe in Figure \ref{fig:tau_analysis_cc} a growth of the energy dissipation when $c_c$ increases. This behavior is what we are expecting since the method becomes more diffusive  when $\tau_c$ is increased due to the last term in (\ref{eq:transfer}). With respect to $c_2$, we observe in Figure \ref{fig:tau_analysis_c12} that decreasing the value of this constant, the energy dissipation is increased. Any effect of the constant $c_1$ is observed in the energy evolution keeping $c_c$ fixed.
%%{\color{blue} 
...
%%} 
%
%In particular, for high Reynolds number problems, the constant $c_1$ does not have so much influence on $\tau_m$, but it does on $\tau_c$. With respect to $c_2$, we observe that it influences the energy dissipation of the method, which is increased when the value of this constant is decreased. When $\tau_c$ is activated ($c_c=1)$, we observe a growth of the energy dissipation when the coefficient $c_2/c_1$ increases. This behavior is what we are expecting since the method becomes more diffusive  when $\tau_c$ is increased due to the last term in (\ref{eq:transfer}).
%%In particular, for high Reynolds number problems, the constant $c_1$ does not have so much influence on $\tau_m$, as it can be seen in  Fig.~\ref{fig:ene_tauc0} where $c_c=0$, but it does on $\tau_c$ (when $c_c=1$ in (\ref{eq:tau_c_cc_new})). With respect to $c_2$, we observe in Fig.~\ref{fig:ene_tauc0} that it influences the energy dissipation of the method, which is increased when the value of this constant is decreased. When $\tau_c$ is activated ($c_c=1)$, we observe a growth of the energy dissipation when the coefficient $c_2/c_1$ increases, see Fig.~\ref{fig:ene_tauc1}. This behavior is what we are expecting since the method becomes more diffusive  when $\tau_c$ is increased due to the last term in (\ref{eq:transfer}).
%
...
%($c_1=12$, $c_2=2$ and $\tau_c=0$) against the setting of the incompressible case in \cite{Avila2014} ($c_1=12$, $c_2=2$ and $\tau_c$ as in (\ref{eq:tau_c})) and a less dissipative setting with $c_1=12$, $c_2=8$ and $\tau_c=0$. These tests have been done using the Dyn-Nl-OSS case in a $32^3$ $Q1$ mesh.
%
%%In order to go in depth about the effect of the algorithmic constants $c_1$ and $c_2$ and the stabilization parameter $\tau_c$ of the incompressibility equation,  we compare the results for the TCF problem with a friction Reynolds number ${\rm Re}_\tau=180$ using the setting used until now ($c_1=12$, $c_2=8$ and $\tau_c=0$) against  the setting of the incompressible case in \cite{Avila2014} ($c_1=12$, $c_2=2$ and $\tau_c$ as in (\ref{eq:tau_c})) and an intermediate setting with $c_1=12$, $c_2=2$ and $\tau_c=0$.
%
%\begin{figure}[h!]
%	\centering	
...
%\end{figure}
%
%In Fig. \ref{fig:OSS_taus_umean} the mean velocity in the streamwise direction is shown. As in the case of homogeneous turbulence, some differences between the three cases can be observed, the choice used in section \ref{sec:TCF} being the most accurate one. The effect of the algorithmic constant $c_2$ and the stabilization parameter $\tau_c$ in the problem solution can be clearly observed, i.e., the less dissipative choice gives the best results.
...
%
%
%%There can be seen that for the fluctuations in the streamwise direction a less dissipative method gives better results. However, for spanwise and wall-normal directions the value of the velocity fluctuations decrease when the dissipation of the method is increased. These results make sense since for a less dissipative method the flow is more unstable and has more oscillations, particularly in the directions where the mean velocity is zero this behavior give grater velocity fluctuations.
%
%\section{Behavior in the small time step limit}
%\label{sec:small_time_step}
%
%Small time step instabilities for VMS LES simulations of turbulent flows have been reported in \cite{Hsu2010,gamnitzer_time-dependent_2010}. In these references, the VMS models differ from the ones in this work. Instead of the definition of  $\tau_m$ in (\ref{eq:tau_m}), a time step dependent stabilization parameter $\tau_m$
%$$
...
%scales with $\delta t$, as discussed in Section (\ref{sec:discrete}). {\it However}, this dependence comes from a consistent time integration of the subscale time derivative (see also \cite[Section 3.2]{codina_time_2007}).} The plain introduction of a time step dependency in $\tau_m$ faces serious difficulties:
%\begin{itemize}
%\item The method becomes unstable in the small time step limit since it converges to the unstable Galerkin formulation.
%%\item A similar situation occurs in the Navier-Stokes problem if $\tau_c=0$.
%\item If $\tau_c$ is computed from (\ref{eq:tau_c}) (as it is usually done, see, e.g., \cite{bazilevs_variational_2007,Hsu2010,gamnitzer_time-dependent_2010,gravemeier_algebraic_2010}), $\tau_m \sim \delta t$ and $\tau_c \sim \delta t^{-1}$ in the small time step limit. If this approach is followed , the essential numerical dissipation given by the fifth term in (\ref{eq:static_energy}) is reduced as $\delta t \to 0$, whereas the numerical dissipation introduced by the last term in the left hand side (a incompressibility penalty term) of (\ref{eq:static_energy}) is increased. It has a compensating effect in practice, but the penalty term does not properly act as a turbulence model. % , as shown in the previous section.
%\end{itemize}
%%The behavior of the VMS models considered herein in the small time step limit is evaluated numerically in Section \ref{sec:TCF}.
%
%Let us perform a test to study the small time step behavior of the VMS methods presented in Section \ref{sec:formulation}, using the skew-symmetric \textit{type 1} form of the convective term, as in previous numerical experiments. We also include a combination we do not advocate here, static subscales and nonlinear splitting, an approach followed in \cite{Calo_2004,bazilevs_variational_2007,Hsu2010,gamnitzer_time-dependent_2010,gravemeier_algebraic_2010}. %The properties of the different methods in this respect have been discussed in Section \ref{sec:small_time_step}. 
%The behavior of all the methods for the TCF test with $\delta t=0.002$ is summarized in Table \ref{table:small_time_step_T}, where YES means that the simulation was successful, NO means that the simulation diverged and $\delta t\downarrow$ means that the simulation was successful only when the adaptive time step strategy described in Section \ref{subsubsec:DHIT_setting} was used.
%
%%It is worth to point out that all these results have been obtained using the skew-symmetric \textit{type 1} form of the convective term, which exactly conserves energy. %Either a conservative or non conservative form is usually employed mentioned references and none of this forms conserve energy. As we have shown in Section \ref{subsubsec:ene_cons_DHIT} for the skew-symmetric \textit{type 2}, they could be productive. 
%It is important to note that the static and nonlinear ASGS formulation used in \cite{bazilevs_variational_2007,Hsu2010,gamnitzer_time-dependent_2010,gravemeier_algebraic_2010} with the convective term \textit{type 2} becomes unstable after some time, as also reported in these works, even for the time step size defined in section \ref{subsubsec:TCF_setting}. However, using the the skew-symmetric \textit{type 1} form of the convective term, which exactly conserves energy, the simulation ended successfully for the time step defined in section \ref{subsubsec:TCF_setting}, but failed to converge with the small one. This result is a numerical evidence of the fact that the use of convective terms without the skew-symmetric property produce energy (see also Section \ref{subsubsec:Total_ene_DHIT}) that can make simulations unstable. Further, \emph{these results evidence once again that it is a good choice to stick to provably unconditionally stable formulations, i.e., the dynamic formulations and/or orthogonal subscales formulations with a skew-symmetric convective term}. %{\color{blue} 
...
%
%
%The methods proposed are different to those whose testing in turbulent regimes has been published before, the closest ones being those reported in \cite{bazilevs_variational_2007,gamnitzer_time-dependent_2010}. First, we consider orthogonal subscales formulations. Further, in \cite{bazilevs_variational_2007} the ASGS method with quasi-static subscales is used (in the isogeometrical analysis context) but the time step dependency is included in the stabilization parameter (with the inconsistencies and problems discussed in section \ref{sec:small_time_step}) and the nonlinear scale splitting is applied in the FE equation only (not in the subscale equation). Time dependent subscales are used in \cite{gamnitzer_time-dependent_2010}, but the authors consider a linear scale splitting. Furthermore, in both works $\tau_c \neq 0$. % but computed from (\ref{eq:tau_c}) with the important consequences we have reported.
%%and only the ASGS method with quasi-static subscales is close to the method tested in \cite{bazilevs_variational_2007}. 
%%We have checked the numerical performance of using dynamic subscales and also the OSS formulation, with results favorable to these options. 
%%{\color{red} I am not sure about this comment, what about Gravemeier's work?... they also used dynamic subscales...}
%
...
%
%However, the most important conclusions come from the different problems that we have solved numerically. Overall, OSS and ASGS yield similar results, all displaying the features of turbulent flows, reproducing appropriately global outputs such as energy spectra. The methods are stable and converge to reference solutions, both when the mesh is refined and when the polynomial order is increased. %Regarding this point, let us mention that we have found a good compromise to use quadratic elements, with better results than for linear elements with the same number of degrees of freedom. %Likewise, the OSS method with dynamic subscales could be the formulation to be favored, since it is the one that introduces (slightly) less dissipation and shows better results in the time evolution of global energy, completely free of oscillations.
%
%%{\color{blue}
%
...
%%Having studied only homogeneous turbulence and channel flow one can conjeture that the constant $c_2$ might need to be lowered in wall flows but the near wall behaviour of VMS methods is something that needs further research.
%%}
