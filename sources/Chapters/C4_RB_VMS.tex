...
%
%Let us consider a two-scale decomposition of spaces $\mathcal{V}_0$ and $\mathcal{Q}_0$ such that $\mathcal{V}_0=\mathcal{V}_{0,h}\oplus\widetilde{\mathcal{V}}_0$ and $\mathcal{Q}=\mathcal{Q}_{0,h}\oplus\widetilde{\mathcal{Q}}_0$, where $\widetilde{\mathcal{V}}_0$ and $\widetilde{\mathcal{Q}}_0$ are infinite-dimensional spaces that complete the FE spaces in $\mathcal{V}_0$ and $\mathcal{Q}_0$, respectively. Hereinafter the subscript $(\cdot)_h$ will denote the FE component and the tilde $\widetilde{(\cdot)}$ the subgrid component. Applying the two-scale decomposition to (\ref{eq-C4_NS_weak}) we obtain a discrete problem
%\begin{equation}
...
%\end{equation}
%where $(\cdot,\cdot)_h=\sum_{K\in\mathcal{T}_h}(\cdot,\cdot)_K$ is the sum of scalar products (\ref{eq-C4_scalar_product}) over each element $K$ of the partition $\mathcal{T}_h$, and
%\begin{equation}
%\label{eq-C4_adjoint}
...
%\end{align}
%In (\ref{eq-C4_velo_sgs})-(\ref{eq-C4_press_sgs}) $\mathcal{P}$ denotes the projection onto the space of subscales, which is discussed below. In turn, the vector $\R$ is the residual of the Navier-Stokes equations (\ref{eq-C4_NS_strong_mome})-(\ref{eq-C4_NS_strong_inc}), defined as $\R=[\R_u,R_p]^T$, with
%\begin{align}
%\label{eq-C4_Ru}
...
%\end{align}
%where $h$ is the mesh size and $c_1$ and $c_2$ are algorithmic constants. Let us comment on expression~(\ref{eq-C4_tau_m}):
%\begin{itemize}
...
%\item The definition of $\tau_m$ in~(\ref{eq-C4_tau_m}) is not standard, in the sense that the one used often depends on the time step size of the time discretization, $\delta t$. Instead of~(\ref{eq-C4_tau_m}), $\tau_m^{-1}=\frac{1}{\delta t} + \frac{c_1\nu}{h^2}+\frac{c_2|\a|}{h}$ is more often considered (see, e.g., \cite{Hsu2010,gamnitzer_time-dependent_2010}). We refer to 
...
%\item Expression~(\ref{eq-C4_tau_m}) corresponds to linear isotropic elements. If elements of order $p$ are used ($p$ is not the pressure, here), $c_1$ must be replaced by $c_1 p^4$ and $c_2$ by $c_2 p$. For anisotropic elements, the definition of $h$ within each element is not obvious. A possibility is explained in \cite{Principe2010}.
%\end{itemize}
%
...
%\begin{equation}
%\label{eq-C4_static_sgs}
%\tilde{\u}=\tau_m \mathcal{P}(\R_u),
%\end{equation}
%that is, to neglect the temporal derivative of the subscales in (\ref{eq-C4_velo_sgs}). In this case, the subscales are called quasi-static in what follows.
%
...
%
%Equation (\ref{eq-C4_velo_sgs}) can be analytically integrated to give
%\begin{equation}
%\label{eq-C4_exact_sgs}
...
%%The vector $\a$ appearing in (\ref{eq-C4_bilinear}) and (\ref{eq-C4_adjoint}) is the advection velocity of the convective term. It can be defined in two different ways depending if we use a linear or a nonlinear scheme for the subscales definition. That is, 
%\begin{align}
%\label{eq-C4_a_lin}
...
%\begin{remark}
%\label{rem:skewsym}
%When we use the nonlinear definition for the advection velocity, $\a=\u_h+\tilde{\u}$, the skew-symmetric term \textit{type 2} (\ref{eq-C4_b_skew2}) in the FE equation (\ref{eq-C4_NS_discrete}) reads: 
%\begin{equation}
%\label{eq-C4_b_skew2_aux1}
%b(\a,\u_h,\v_h)=((\u_h+\tilde{\u})\cdot\nabla\u_h,\v_h)+\frac{1}{2}(\u_h\cdot\v_h,\nabla\cdot\u_h)+\frac{1}{2}(\u_h\cdot\v_h,\nabla\cdot\tilde{\u}).
%\end{equation}
%The last term is not well-defined, since it includes derivatives of the discontinuous subscale $\tilde{\u}$. One possibility is to neglect it (as previously done with other similar terms when arriving to (\ref{eq-C4_NS_discrete})), which implies
%\begin{equation}
%b(\a,\u_h,\u_h)=-\frac{1}{2}(|\u_h|^2,\nabla\cdot\tilde{\u}),
%\end{equation}
%the same result obtained when the non conservative form is used.
%By contrast, the skew-symmetric term \textit{type 1} in the FE equation (\ref{eq-C4_NS_discrete}) reads
%\begin{equation}
%\label{eq-C4_b_skew1_aux1}
...
%- (\tilde\u\otimes\tilde\u, \nabla\v_h).\label{eq-C4_nonconconv}
%\end{align}
%Even if this is not exactly what we get using the non-conservative or skew-symmetric forms because of the approximation error, this allows us to interpret the different contributions arising from the nonlinear scale splitting. As it is explained in \cite{Codina-chap-2011}, from (\ref{eq-C4_nonconconv}) we can identify the contributions from the cross stresses, the Reynolds stresses and the subgrid scale tensor. 
%\end{remark}
%
%\subsubsection{The space for the subscales}
%\label{sec-oss}
%The selection of the space for the approximation of the subscales determines the projection $\mathcal{P}$ appearing in the right-hand side of (\ref{eq-C4_velo_sgs}) and (\ref{eq-C4_press_sgs}). The first option, already considered in \cite{Hughes2000,hughes_large_2001,bazilevs_variational_2007} and named Algebraic Subgrid Scale (ASGS) in \cite{codina_stabilization_2000} is to take the subscales in the space of the residuals, that is,
...
%
%%this definition should be changed modifying the weight in a way that depends on the time integration scheme. For example if equation (\ref{eq-C4_velo_sgs}) is discretized as $\tilde{\u}^{n+1}=\tau_m \mathcal{P}(\R_u) + \frac{1}{\delta t}\tilde{\u}^n$ the projection \ref{eq-C4_Pi_tau_m} must be weighted using $\left(\tau_m^{-1}+{\delta t}^{-1}\right)^{-1}$ instead of  $\tau_m$. It is important to remark that using the weighted projections the residual of the monentum equation does depend on $\partial_t\u_h$. {\color{blue} Oriol, como esta en el codigo? Conservamos la derivada temporal al calcular las proyecciones?}
%
...
%
%%In parallel, small time step instabilities were also reported in \cite{Hsu2010,gamnitzer_time-dependent_2010} although the VMS model used is different in that introduces a time step dependency on the stabilization parameter $\tau_m$. This dependency actually appears in the final discrete problem when dynamic subscales are used, as the final parameter $\tau_{m,t}$ scales with $\delta t$, as discussed in Section \ref{sec-discrete}. {\it However} this happens in conjunction with a term that comes from the discretization of a temporal derivative in (\ref{eq-C4_velo_sgs}), see also \cite[Section 3.2]{codina_time_2007}. The plain introduction of a time step dependency in $\tau_m$ faces serious difficulties.
%%\begin{itemize}
...
%%\item If $\tau_c$ is computed from (\ref{eq-C4_tau_c}), as it is usually done, in the small time step limit $\tau_m \sim \delta t$ and $\tau_c \sim \delta t^{-1}$. If this commonly used approach is followed \cite{bazilevs_variational_2007,Hsu2010,gamnitzer_time-dependent_2010,gravemeier_algebraic_2010}, when the time step is diminished the essential numerical dissipation given by the fifth term in (\ref{eq-C4_static_energy}) is reduced but the numerical dissipation introduced by the last term in the left hand side of (\ref{eq-C4_static_energy}) is increased which can be a compensating effect in practice.
%%\end{itemize}
%
...
%%\end{equation}
%Following Rogallo \cite{rogallo_numerical_1981}, we generate the initial field on the Fourier space such that it satisfies continuity and has the energy spectrum prescribed in (\ref{eq-C4_ini_E_DHIT1}). 
%% First, we define two complex parameters:
%% \begin{align}
%% \label{eq-C4_alpha_E0_DHIT}
%% &\alpha=\left(\frac{E(k,0)}{2\pi k^2}\right)^{1/2}\exp(i\theta_1)\cos(\phi),\\
%% \label{eq-C4_beta_E0_DHIT}
...
%% To have the field (\ref{eq-C4_ini_FFTu_DHIT}) in the physical space we apply the inverse Fast Fourier Transform.
...
%
%% In Figure \ref{Fig:ini_spec_DHIT} we show the initial analytical energy spectrum and the one calculated from the initial velocity field on the Fourier space (\ref{eq-C4_ini_FFTu_DHIT}) using a $128^3$ linear elements mesh. We observe that the computed energy spectra fits to the analytical one. It is what we expect since we use the initial field proposed by Rogallo \cite{rogallo_numerical_1981}, which precisely satisfies this condition.
%
%% \begin{figure}[h!]
%% 	\centering	
...
%%The initial energy spectrum is the defined in (\ref{eq-C4_ini_E_DHIT2}) with a peak on the wavenumber $k_0=6$ which supposes that the total kinetic energy at the initial time step should be $1/2q^2=1.5$ and the enstrophy $\frac{1}{2}<w^2>=67.5$.
%
...
%\end{equation}
%%These experiments have been done with the DHIT test using the Dyn-Nl-OSS case in a $32^3$ $Q$1 mesh and are depicted in Figure \ref{fig:tau_analysis}. They show some changes in the behavior of the VMS methods in terms of energy dissipation. In particular, for high Reynolds number problems, the constant $c_1$ does not have so much influence on $\tau_m$, but it does on $\tau_c$ (when $c_c$ is defined as in (\ref{eq-C4_tau_c}), $c_c=1/c_1$). We observe in Figure \ref{fig:tau_analysis_cc} a growth of the energy dissipation when $c_c$ increases. This behavior is what we are expecting since the method becomes more diffusive  when $\tau_c$ is increased due to the last term in (\ref{eq-C4_transfer}). With respect to $c_2$, we observe in Figure \ref{fig:tau_analysis_c12} that decreasing the value of this constant, the energy dissipation is increased. Any effect of the constant $c_1$ is observed in the energy evolution keeping $c_c$ fixed.
%%{\color{blue} 
...
%%In particular, for high Reynolds number problems, the constant $c_1$ does not have so much influence on $\tau_m$, as it can be seen in  Fig.~\ref{fig:ene_tauc0} where $c_c=0$, but it does on $\tau_c$ (when $c_c=1$ in (\ref{eq-C4_tau_c_cc_new})). With respect to $c_2$, we observe in Fig.~\ref{fig:ene_tauc0} that it influences the energy dissipation of the method, which is increased when the value of this constant is decreased. When $\tau_c$ is activated ($c_c=1)$, we observe a growth of the energy dissipation when the coefficient $c_2/c_1$ increases, see Fig.~\ref{fig:ene_tauc1}. This behavior is what we are expecting since the method becomes more diffusive  when $\tau_c$ is increased due to the last term in (\ref{eq-C4_transfer}).
%
